    ...
    double sizeSortedFractionOfSurfaceAreaDeadOrLive[FuelConstants::MaxParticles])
{
    ...
    for (int i = 0; i < FuelConstants::MaxParticles; i++)
    {
        //sizeSortedFractionOfSurfaceAreaDeadOrLive[i] = fractionOfTotalSurfaceAreaDeadOrLive[i];
...
double SurfaceFuelbedIntermediates::getTotalLiveHerbaceousFuelLoad(LoadingUnits::LoadingUnitsEnum loadingUnits) const {

    return LoadingUnits::fromBaseUnits(loadLive_[0], loadingUnits);
}
